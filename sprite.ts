...
class Sprite {
    ...
    public draw(windowLeft: number, windowRight: number, windowTop: number[], windowBottom: number[]) {
        ...
        for (let x = x0; x < x1; x++) {
            ...
            let y0 = Math.max(topY, windowTop[x - windowLeft]);
            let y1 = Math.min(bottomY, windowBottom[x - windowLeft]);
